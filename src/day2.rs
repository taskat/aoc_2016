...
impl common::Puzzle for Puzzle {
    fn part_1(&self, input: String) -> String {
        ...
        code.iter().map(|number| number.to_string()).collect::<String>()
    }
...
fn next_simple(start: i32, line: String) -> i32 {
    ...
    for char in line.chars() {
        match char {
            'U' => if current > 3 {current -= 3},
            'R' => if current % 3 > 0 {current += 1},
            'D' => if current < 7 { current += 3},
            'L' => if current % 3 != 1 {current -= 1},
...
fn next_complex(start: char, line: String) -> char {
    ...
    let neighbors: HashMap<char, HashMap<char, char>> = HashMap::from([
        ...
        ('3', HashMap::from([('U', '1'), ('R', '4'), ('D', '7'), ('L', '2')])),
        ('4', HashMap::from([('D', '8'), ('L', '3')])),
        ('5', HashMap::from([('R', '6')])),
        ('6', HashMap::from([('U', '2'), ('R', '7'), ('D', 'A'), ('L', '5')])),
        ('7', HashMap::from([('U', '3'), ('R', '8'), ('D', 'B'), ('L', '6')])),
        ('8', HashMap::from([('U', '4'), ('R', '9'), ('D', 'C'), ('L', '7')])),
        ('9', HashMap::from([('L', '8')])),
        ('A', HashMap::from([('U', '6'), ('R', 'B')])),
        ('B', HashMap::from([('U', '7'), ('R', 'C'), ('D', 'D'), ('L', 'A')])),
...
mod tests {
    ...
    fn part_1() {
        ...
        for case in cases {
            let solution =
                crate::day2::Puzzle {}.part_1(read_input(&FakeConfig::new(2, 1, case.0)).unwrap());
    ...
    fn part_2() {
        ...
        for case in cases {
            let solution =
                crate::day2::Puzzle {}.part_2(read_input(&FakeConfig::new(2, 2, case.0)).unwrap());
            assert_eq!(solution, case.1);
        }
    }
}